...
describe("Logger", function()
  ...
  it("can flush log buffer on startup", function()
    local tempAddon = builder:Build({ LOG_LEVEL = 4 })
  ...
  it("can set the global log level", function()
    local tempAddon = builder:Build({ LOG_LEVEL = 4, LOG_MODE = "simple-unbuffered" })
    tempAddon:Init()
    tempAddon:Advance()
    local printSpy = spy.on(_G, "print")
    tempAddon.Logger:Debug("outside of log level")
    assert.spy(printSpy).was_not_called()
    tempAddon:SetGlobalLogLevel("debug")
    tempAddon.Logger:SetLogLevel("debug")
    print:clear()
    tempAddon.Logger:Debug("within log level")
    assert.spy(printSpy).was_called()
  end)
end)
