...
Line C]],
  ...
  describe("ParseIdOrName", function()
    it("can parse id from number", function()
      local id, name = addon:ParseIdOrName(1234)
      assert.equals(1234, id)
      assert.is_nil(name)
    end)
    it("can parse id from string representation of number", function()
      local id, name = addon:ParseIdOrName("1234")
      assert.equals(1234, id)
      assert.is_nil(name)
    end)
    it("can parse name from non-numeric string", function()
      local id, name = addon:ParseIdOrName("Thunderfury")
      assert.is_nil(id)
      assert.equals("Thunderfury", name)
    end)
    it("throws error on nil value", function()
      assert.has_error(function() addon:ParseIdOrName(nil) end)
    end)
    it("throws error on value that is not a number or string", function()
      assert.has_error(function() addon:ParseIdOrName(true) end)
    end)
  end)
end)